...
class Users extends CI_Controller {
    ...
    */
    public function signin() 
    {
        ...
        if(!$current_user_id) { 
            $this->load->view('templates/header');
            $this->load->view('users/signin', $view_data);
        } 
        else {
            redirect("wall");
        }
    ...
    */
    public function process_signin() 
    {
        ...
        else 
        {
            ...
            $result = $this->user->validate_signin_match($user_info, $this->input->post('password'));
            
            if($result == "success") 
            {
                $this->session->set_userdata(array('user_id'=>$user_info['id'], 'first_name'=>$user_info['first_name']));            
                redirect("wall");
            }
            else 
            {
